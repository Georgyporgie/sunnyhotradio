...
function crossfadeToNextTrack() {
  let next_index = (track_index + 1) % trackList.length;
  let next_track = new Audio(trackList[next_index].path);
  next_track.volume = 0;
  next_track.load();
...
function setupTrackUI(index) {
  track_art.style.backgroundImage = "url(" + tracklist[index].image + ")";
  track_name.textContent = trackList[index].name;
  track_artist.textContent = trackList[index].artist;
  now_playing.textContent = "PLAYING " + (index + 1) + " OF " + trackList.length;
...
window.addEventListener("load", () => {
  track_index = 0;
  curr_track = new Audio(trackList[track_index].path);
