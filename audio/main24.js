...
function schedulerTick() {
    ...
    if (item.type === "news") {
        ...
        // Fade out music
        fadeOutCurrentTrack(2000);

        // Play the news bulletin
        playScheduledItem(item);

        // Resume music after news ends
        setTimeout(() => {
            resumePlaylist();
        }, item.duration * 1000);

        return; // IMPORTANT: prevents music logic from running now
    }
...
function fadeOutCurrentTrack(duration = 2000) {
...
function playScheduledItem(item) {
    if (playlistPlayer) playlistPlayer.pause();
